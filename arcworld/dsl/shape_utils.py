...
def shape(piece: Piece) -> Coordinate:
...
def ulcorner(patch: Patch) -> Coordinate:
    """index of upper left corner"""
    return tuple(map(min, zip(*toindices(patch))))


def urcorner(patch: Patch) -> Coordinate:
...
def llcorner(patch: Patch) -> Coordinate:
...
def lrcorner(patch: Patch) -> Coordinate:
    """index of lower right corner"""
    return tuple(map(max, zip(*toindices(patch))))


def crop(grid: Grid, start: Coordinate, dims: Coordinate) -> Grid:
...
# TODO: Correct this type hack.
def toindices(patch: Patch) -> Coordinates:
    """indices of object cells"""
    if len(patch) == 0:
        return cast(Coordinates, frozenset())

    if isinstance(next(iter(patch))[1], tuple):
        # This is the case for which it should be a Shape.
        return cast(Coordinates, frozenset(index for _, index in patch))
...
def neighbors(loc: Coordinate) -> Coordinates:
    """adjacent indices"""
...
def hmatching(a: Patch, b: Patch) -> Boolean:
    ...
    return len(set(i for i, _ in toindices(a)) & set(i for i, _ in toindices(b))) > 0


def vmatching(a: Patch, b: Patch) -> Boolean:
    """whether there exists a column for which both patches have cells"""
    return len(set(j for _, j in toindices(a)) & set(j for _, j in toindices(b))) > 0
...
def centerofmass(patch: Patch) -> Coordinate:
    """center of mass"""
...
def center(patch: Patch) -> Coordinate:
    """center of the patch"""
    return (uppermost(patch) + height(patch) // 2, leftmost(patch) + width(patch) // 2)


def position(a: Patch, b: Patch) -> Coordinate:
...
def index(grid: Grid, loc: Coordinate) -> Integer:
    """color at location"""
    i, j = loc
    h, w = len(grid), len(grid[0])
...
def canvas(value: Integer, dimensions: Coordinate) -> Grid:
    """grid construction"""
    return tuple(
...
def corners(patch: Patch) -> Coordinates:
    """indices of corners"""
    return frozenset(
...
def connect(a: Coordinate, b: Coordinate) -> Coordinates:
...
def vfrontier(location: Coordinate) -> Coordinates:
    """vertical frontier"""
    return frozenset((i, location[1]) for i in range(30))


def hfrontier(location: Coordinate) -> Coordinates:
    """horizontal frontier"""
    return frozenset((location[0], j) for j in range(30))


def backdrop(patch: Patch) -> Coordinates:
...
def gravitate(source: Patch, destination: Patch) -> Coordinate:
...
def inbox(patch: Patch) -> Coordinates:
    """inbox for patch"""
...
def outbox(patch: Patch) -> Coordinates:
    """outbox for patch"""
    ai, aj = uppermost(patch) - 1, leftmost(patch) - 1
    bi, bj = lowermost(patch) + 1, rightmost(patch) + 1
    si, sj = min(ai, bi), min(aj, bj)
...
def box(patch: Patch) -> Coordinates:
    """outline of patch"""
    if len(patch) == 0:
        return patch
...
def shoot(start: Coordinate, direction: Coordinate) -> Coordinates:
